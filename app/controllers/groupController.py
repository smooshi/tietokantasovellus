...
def group(id):
    ...
    users = select_users_in_group(id)
    u_amount = len(users)
    grouped = False
    userNames = list()
    ...
    if (isU[0][0] == 1):
        grouped = True
        for user_id, group_id, isAdmin in users:
            u = select_by_id_user(user_id)
    ...
    return render_template('/group/inspect.html', user=user, group=group, u_amount=u_amount, grouped=grouped, userNames=userNames)